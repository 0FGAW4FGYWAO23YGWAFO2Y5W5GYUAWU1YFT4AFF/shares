    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSendedShare += 1
                if DebugMode == True:
                    print(Colorate.Horizontal(Colors.red_to_purple, f"Sended Share: {TotalSendedShare} ({impr_id})"))
                else:
                    print(Colorate.Horizontal(Colors.red_to_purple, f"Sended Share: {TotalSendedShare} ({impr_id})"))
...
if (__name__ == "__main__"):
    Clear()
    itemID       = Write.Input("Video Link > ", Colors.green_to_white, interval=0.0001)
    amount       = Write.Input("Amount (0=inf) > ", Colors.green_to_white, interval=0.0001)
    NThread      = Write.Input("Thread Amount > ", Colors.green_to_white, interval=0.0001)
    
    if Title("Proy Scrapper X-Proxy by NightFallGT") == True:
        Debug = Write.Input("Debug Fails [y/n] ? > ", Colors.green_to_white, interval=0.0001)
